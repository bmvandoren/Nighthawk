...
def run_detector_on_files(
    ...
    for i, input_file_path in enumerate(input_file_paths):
        ...
        # Model input is always 1 s; export the zero-padded clip so short
        # recordings can be inspected as the model hears them.
...
def _export_zero_padded_audio(
    ...
    """Load audio, center-pad with zeros to input_dur, and write a WAV file.

    Returns the output path, or None if no padding was needed.
    """
...
def _print_empty_detection_reason(
        ...
        short_no_drop=False, file_path=None):
    ...
    if file_dur < MODEL_INPUT_DURATION:
        ...
        if file_path is not None:
            padded_name = f'{Path(file_path).stem}_padded.wav'
            print(
                f'  The padded audio was written as "{padded_name}" '
                '(same directory as other outputs) for inspection.')
